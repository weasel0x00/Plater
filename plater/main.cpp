...
void help()
{
    ...
    cerr << "            -t N runs N anneal chains in parallel (best kept)" << endl;
    cerr << "-B: (anneal) balance pass - even out part volume (a print-time proxy) across" << endl;
    cerr << "    plates when the minimum is >1 plate, so plates take a similar time to print" << endl;
    cerr << "    (runs a 2nd anneal phase)" << endl;
    ...
    cerr << "    tall parts in the middle). With -A anneal the search optimises a centred" << endl;
    cerr << "    layout and composes with -B; otherwise tall parts are centred within the" << endl;
    cerr << "    minimum plate count" << endl;
